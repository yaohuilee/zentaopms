...
window.executeWithFormContext = async function(promptID)
{
    ...
    try
    {
        ...
        if($targetEl)
        {
            ...
            const labelsAttr = $targetEl.attr('data-field-labels');
            if(labelsAttr)
            {
                try
                {
        ...
        if(window.parent && window.parent.executeUniversalPromptWithZentaoAPI)
        {
            ...
            window.parent.executeUniversalPromptWithZentaoAPI(formSchema, contextIDs, promptID, promptFields, allowedFields, agentRole, agentPurpose, isBatch, agentSkills, fieldLabels);
        }
    }
    catch(error)
    {

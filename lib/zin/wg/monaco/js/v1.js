...
function createMonaco(id, action, options, diffContent, onMouseDown, onMouseMove, vsPath, clientLang)
{
    ...
    require(['vs/editor/editor.main'], function ()
    {
        ...
        if(action == 'diff')
        {
            ...
            /* 隐藏修改片段之外的区域，只展示修改的内容。 */
            const hasOldRanges = diffContent.oldRanges && diffContent.oldRanges.length;
            const hasNewRanges = diffContent.newRanges && diffContent.newRanges.length;
            if(hasOldRanges || hasNewRanges)
            {
                if(hasOldRanges)
                {
                    getOriginalEditor.setHiddenAreas(buildHiddenRanges(diffContent.oldRanges, getOriginalEditor.getModel().getLineCount()));
                    getOriginalEditor.revealRangeInCenter(new monaco.Range(diffContent.oldRanges[0][0], 1, diffContent.oldRanges[0][0], 1));
                }
                if(hasNewRanges)
                {
                    editor.setHiddenAreas(buildHiddenRanges(diffContent.newRanges, editor.getModel().getLineCount()));
                    editor.revealRangeInCenter(new monaco.Range(diffContent.newRanges[0][0], 1, diffContent.newRanges[0][0], 1));
                }
            }
...
/**
 * 计算修改片段之外的隐藏区域。
 * Build hidden ranges except changed ranges.
 *
 * @param  array  ranges
 ...
 * @return array
 */
function buildHiddenRanges(ranges, lineCount)
{
    const hidden = [];
    let prev = 1;
    ranges.forEach(range =>
    {
        if(range[0] > prev) hidden.push(new monaco.Range(prev, 1, range[0] - 1, 1));
        prev = range[1] + 1;
    });
    if(prev <= lineCount) hidden.push(new monaco.Range(prev, 1, lineCount, 1));
    return hidden;
}

...
helper::import($tester->app->getModulePath('', 'metric') . 'control.php');
helper::import($tester->app->getModulePath('', 'metric') . 'zen.php');

/**

title=测试 metricModel::getOldMetricInfo()
timeout=0
cid=0
...
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('metric', 'getOldMetricInfo', array(1)); }))) && p() && e('error:Error'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('metric', 'getOldMetricInfo', array(0)); }))) && p() && e('error:Error'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('metric', 'getOldMetricInfo', array('abc')); }))) && p() && e('error:Error'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('metric', 'getOldMetricInfo', array(999999)); }))) && p() && e('error:Error'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('metric', 'getOldMetricInfo', array('test')); }))) && p() && e('error:Error'); // 步骤5：业务规则验证

...
/**
 * The charset step view file of upgrade module of ZenTaoPMS.
 *
...
$items = array();
foreach($charsetTables as $table)
{
    $sql = isset($charsetSQLs[$table]) ? $charsetSQLs[$table] : '';
    $items[] = row
    (
        ...
        (
            ...
            setData(['text' => $lang->upgrade->charset->update]),
            icon(setClass('animate-spin'), 'spinner-indicator')
        ),
        span
        (
            sprintf($lang->upgrade->charset->change, $table)
        ),
        $sql ? a
        (
    ...
    (
        ...
        (
            ...
            (
                ...
                $lang->upgrade->charset->common
            ),
            span
            (
                html(sprintf($lang->upgrade->dataProcessProcessed, "<span id='charsetProcessedCount'>0</span>", $count))
            )
        ),
        col
        (

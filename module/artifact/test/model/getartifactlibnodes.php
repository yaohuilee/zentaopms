...
$result = $testObj->getArtifactLibNodesTest((object)array(), '1');
r(count($result)) && p() && e('0'); // 步骤1：正常输入
$result = $testObj->getArtifactLibNodesTest((object)array(), '1');
r(count($result)) && p() && e('0'); // 步骤2：边界值输入
$result = $testObj->getArtifactLibNodesTest((object)array(), '1');
r(count($result)) && p() && e('0'); // 步骤3：无效输入
r($testObj->getArtifactLibNodesTest((object)array('id' => 999999), '1')) && p() && e('daoError:{"apiMessage":"指定的制品视图不存在."}'); // 步骤4：大值输入
r($testObj->getArtifactLibNodesTest((object)array('id' => 1, 'name' => 'test'), 'test')) && p() && e('daoError:{"apiMessage":"指定的制品视图不存在."}'); // 步骤5：业务规则验证

...
- 第 2 条记录的用户名是 user1，权限组 id 是 2。
 ...
 - 第1条的group属性 @2
- 查看用户视图，返回 2 条记录。 @2
- 第 1 条记录的用户名是 admin。 @1
- 第 2 条记录的用户名是 user1。 @1
...
$views = $tester->dao->select('*')->from(TABLE_USERVIEW)->fetchAll('account');
r(count($views))          && p() && e(2);   // 查看用户视图，返回 2 条记录。
r(isset($views['admin'])) && p() && e('1'); // 第 1 条记录的用户名是 admin。
r(isset($views['user1'])) && p() && e('1'); // 第 2 条记录的用户名是 user1。
...
r(count($views))           && p() && e(2);    // 查看用户视图，返回 2 条记录。
r(isset($views['admin']))  && p() && e('1');  // 第 1 条记录的用户名是 admin。
r(isset($views['user12'])) && p() && e('1');  // 第 2 条记录的用户名是 user12。
...
$histories = $tester->dao->select('*')->from(TABLE_HISTORY)->where('action')->eq($action->id)->fetchAll('', false);
r(count($histories)) && p()                  && e(1);               // 创建历史记录成功，最后一条日志的历史记录是 1 条。
r($histories[0])     && p('field,old,new') && e('account,user1,user12'); // 创建历史记录成功，最后一条日志的历史记录的字段是 account，旧值是 user1，新值是 user12。

...
window.buildColCardActions = function(col)
{
    ...
    if(kanban.object && kanban.object != 'cards' && vision != 'lite')
    {
        ...
        if(kanban.object.indexOf('tickets') != -1) items.push({text: kanbanLang.importTicket, url: $.createLink('kanban', 'importTicket', `kanbanID=${kanbanID}&regionID=${col.region}&groupID=${col.group}&columnID=${col.id}`), 'data-toggle': 'modal'});
        actions.push(
            {

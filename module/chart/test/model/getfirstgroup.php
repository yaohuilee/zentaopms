...
$zd_module = zenData('module');
$zd_module->id->range('1-5');
$zd_module->branch->range('0');
$zd_module->parent->range('0');
$zd_module->from->range('0');
$zd_module->deleted->range('0');
$zd_module->gen(5);
...
r(ztfVal(ztfCall(function() use ($tester) { return $tester->chart->getFirstGroup(1); }))) && p() && e('0'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->chart->getFirstGroup(0); }))) && p() && e('0'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->chart->getFirstGroup(-1); }))) && p() && e('0'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->chart->getFirstGroup(999999); }))) && p() && e('0'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->chart->getFirstGroup(2); }))) && p() && e('0'); // 步骤5：业务规则验证

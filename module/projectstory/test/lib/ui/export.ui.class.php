...
class exportTester extends tester
{
    ...
    public function export($projectStory = array())
    {
        ...
        if(isset($projectStory['format']))   $form->dom->fileType->picker($projectStory['format']);
        if(isset($projectStory['encoding'])) $form->dom->encode->picker($projectStory['encoding']);
        if(isset($projectStory['data']))     $form->dom->exportType->picker($projectStory['data']);

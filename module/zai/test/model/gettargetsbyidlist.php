...
title=测试 zaiModel::getTargetsByIDList()
timeout=0
cid=0
...
r(ztfVal(ztfCall(function() use ($tester) { return $tester->zai->getTargetsByIDList('http://127.0.0.1:1/', array()); }))) && p() && e('empty_array'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->zai->getTargetsByIDList('http://127.0.0.1:1/', array()); }))) && p() && e('empty_array'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->zai->getTargetsByIDList('http://127.0.0.1:1/', array()); }))) && p() && e('empty_array'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->zai->getTargetsByIDList('http://127.0.0.1:1/', array()); }))) && p() && e('empty_array'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->zai->getTargetsByIDList('http://127.0.0.1:1/', array(1, 2)); }))) && p() && e('empty_array'); // 步骤5：业务规则验证

...
title=测试 myModel::deleteSSH()
timeout=0
cid=0

- 步骤1：正常输入 @daoError:{"apiMessage":"公钥不存在，删除失败"}
- 步骤2：边界值输入 @daoError:{"apiMessage":"公钥删除路径参数解析失败"}
- 步骤3：无效输入 @daoError:{"apiMessage":"公钥删除路径参数解析失败"}
- 步骤4：大值输入 @daoError:{"apiMessage":"公钥不存在，删除失败"}
- 步骤5：业务规则验证 @daoError:{"apiMessage":"公钥不存在，删除失败"}

*/

r(ztfVal(ztfCall(function() use ($tester) { return $tester->my->deleteSSH(1); }))) && p() && e('daoError:{"apiMessage":"公钥不存在，删除失败"}'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->my->deleteSSH(0); }))) && p() && e('daoError:{"apiMessage":"公钥删除路径参数解析失败"}'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->my->deleteSSH(-1); }))) && p() && e('daoError:{"apiMessage":"公钥删除路径参数解析失败"}'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->my->deleteSSH(999999); }))) && p() && e('daoError:{"apiMessage":"公钥不存在，删除失败"}'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->my->deleteSSH(2); }))) && p() && e('daoError:{"apiMessage":"公钥不存在，删除失败"}'); // 步骤5：业务规则验证

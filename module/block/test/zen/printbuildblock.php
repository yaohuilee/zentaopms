...
$zd_build = zenData('build');
$zd_build->id->range('1-5');
$zd_build->artifactRepoID->range('0');
$zd_build->createdDate->range('(M)-(w)')->type('timestamp')->format('YYYY-MM-DD hh:mm:ss');
$zd_build->deleted->range('0');
$zd_build->gen(5);
...
title=测试 blockModel::printBuildBlock()
timeout=0
cid=0
...
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('block', 'printBuildBlock', array((object)array())); }))) && p() && e('error:Error'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('block', 'printBuildBlock', array((object)array())); }))) && p() && e('error:Error'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('block', 'printBuildBlock', array((object)array())); }))) && p() && e('error:Error'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('block', 'printBuildBlock', array((object)array('id' => 999999))); }))) && p() && e('error:Error'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return callZenMethod('block', 'printBuildBlock', array((object)array('id' => 1, 'name' => 'test'))); }))) && p() && e('error:Error'); // 步骤5：业务规则验证

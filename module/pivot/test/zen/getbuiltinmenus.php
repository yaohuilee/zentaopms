#!/usr/bin/env php
<?php

include dirname(__FILE__, 5) . '/test/lib/init.php';
include dirname(__FILE__, 2) . '/lib/pivotzen.unittest.class.php';

error_reporting(E_ERROR);

$zd_user = zenData('user');
$zd_user->id->range('1-1');
$zd_user->account->range('1-1');
$zd_user->last->range('(M)-(w)')->type('timestamp')->format('YYYY-MM-DD hh:mm:ss');
$zd_user->feedback->range('0');
$zd_user->scoreLevel->range('0');
$zd_user->resetExpired->range('0');
$zd_user->jira->range('0');
$zd_user->deleted->range('0');
$zd_user->gen(1);

su('admin');

$tester->loadModel('pivot');
helper::import($tester->app->getModulePath('', 'pivot') . 'control.php');
helper::import($tester->app->getModulePath('', 'pivot') . 'zen.php');


$testObj = new pivotZenTest();
/**

title=测试 pivotModel::getBuiltinMenus()
timeout=0
cid=0

- 步骤1：正常输入 @0
- 步骤2：边界值输入 @0
- 步骤3：无效输入 @0
- 步骤4：大值输入 @0
- 步骤5：业务规则验证 @0

*/

$result = $testObj->getBuiltinMenusTest(1, (object)array());
r(count($result)) && p() && e('0'); // 步骤1：正常输入
$result = $testObj->getBuiltinMenusTest(0, (object)array());
r(count($result)) && p() && e('0'); // 步骤2：边界值输入
$result = $testObj->getBuiltinMenusTest(-1, (object)array());
r(count($result)) && p() && e('0'); // 步骤3：无效输入
$result = $testObj->getBuiltinMenusTest(999999, (object)array());
r(count($result)) && p() && e('0'); // 步骤4：大值输入
$result = $testObj->getBuiltinMenusTest(2, (object)array('id' => 1, 'name' => 'test'));
r(count($result)) && p() && e('0'); // 步骤5：业务规则验证

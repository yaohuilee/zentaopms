...
$tester->loadModel('runner');

/**

title=测试 runnerModel::getList()
timeout=0
cid=0
...
r(ztfVal(ztfCall(function() use ($tester) { return $tester->runner->getList('http://127.0.0.1:1/', null); }))) && p() && e('error:EndResponseException'); // 步骤1：正常输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->runner->getList('http://127.0.0.1:1/', null); }))) && p() && e('error:EndResponseException'); // 步骤2：边界值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->runner->getList('http://127.0.0.1:1/', null); }))) && p() && e('error:EndResponseException'); // 步骤3：无效输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->runner->getList('http://127.0.0.1:1/', null); }))) && p() && e('error:EndResponseException'); // 步骤4：大值输入
r(ztfVal(ztfCall(function() use ($tester) { return $tester->runner->getList('http://127.0.0.1:1/', (object)array('id' => 1, 'name' => 'test')); }))) && p() && e('error:EndResponseException'); // 步骤5：业务规则验证
